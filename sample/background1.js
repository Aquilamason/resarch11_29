   ...
   */
    ...
    */
	...
	function parseCMSSigned() {
		...
		var asn1 = fromBER(cmsSignedBuffer);
		var cmsContentSimpl = new ContentInfo({ schema: asn1.result });
		var cmsSignedSimpl = new SignedData({ schema: cmsContentSimpl.content });
		//endregion
		...
		for (var i = 0; i < cmsSignedSimpl.digestAlgorithms.length; i++) {
			var typeval = dgstmap[cmsSignedSimpl.digestAlgorithms[i].algorithmId];
		...
		var eContentType = contypemap[cmsSignedSimpl.encapContentInfo.eContentType];
		if (typeof eContentType === "undefined") eContentType = cmsSignedSimpl.encapContentInfo.eContentType;
		...
		if ("crls" in cmsSignedSimpl) {
			for (var _j10 = 0; _j10 < cmsSignedSimpl.crls.length; _j10++) {
				...
				var _row = crlsTable.insertRow(certificatesTable.rows.length);
	...
	/* add by miyamoto read data do createCMSSigned*/
	...
	function verifyCMSSignedInternal() {
		//region Initial check
		if (cmsSignedBuffer.byteLength === 0) return Promise.reject("Nothing to verify!");
		//endregion
	...
	function verifyCMSSigned() {
		...
		return verifyCMSSignedInternal().then(function (result) {
			return alert("Verification result: " + result);
		}, function (error) {
	...
	context("CMS Signed Complex Example", function () {
		...
		var addExts = [false, true];
		var detachedSignatures = [false, true];
			...
			hashAlgs.forEach(function (_hashAlg) {
				addExts.forEach(function (_addExt) {
					detachedSignatures.forEach(function (_detachedSignature) {
						var testName = _hashAlg + " + " + _signAlg + ", add ext: " + _addExt + ", detached signature: " + _detachedSignature;
